...
export async function middleware(request: NextRequest) {
  ...
  if (!user && (path.startsWith('/dashboard') || path.startsWith('/check-in') || path.startsWith('/history') || path.startsWith('/admin'))) return NextResponse.redirect(new URL('/login', request.url));
